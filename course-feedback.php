...
try {
    ...
    $required = ['name', 'enrollment', 'year', 'department', 'semester', 'section', 'subject'];
    $missing = [];
    ...
    $check_sql = "SELECT COUNT(*) as count FROM course_feedback WHERE enrollment_number = ? AND subject = ?";
    ...
    $insert_sql = "INSERT INTO course_feedback (
        name, enrollment_number, academic_year, department, semester, section,
    ...
    $bind_types = "ssssssss"; // First 8 string params
    $bind_params = [
        $data['name'], $data['enrollment'], $data['year'], $data['department'],
        $data['semester'], $data['section'], $data['subject'], $data['subject_name']
    ];
